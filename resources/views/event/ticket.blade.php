...
<header class="fixed top-0 w-full flex items-center justify-between px-4 sm:px-10 py-5 font-poppins bg-dark-background/50 backdrop-blur-xl z-50">
    ...
    @forelse ($tickets as $ticket)
        <div class="max-w-4xl mx-auto bg-light-secondary dark:bg-dark-secondary rounded-xl shadow-lg mb-8 overflow-hidden transition transform hover:scale-[1.01] duration-300">
            <div class="grid sm:grid-cols-3">
                ...
                <div class="col-span-2 p-6 space-y-4">
                    <h2 class="text-2xl font-semibold dark:text-white">{{ $ticket->event->title }}</h2>
                    <h2 class="text-sm font-semibold dark:text-white">{{ $ticket->user->name }}</h2>
                    
                    <p class="text-base text-gray-700 dark:text-gray-300">{{ $ticket->event->description }}</p>
